...
const permit = new Permit({
    ...
    // your api key
    token: "permit_key_HIXwnguJ4WrcAH5LD6YlzWgGxV2NcWKZgyy5tgLUBjjHexaJipdlDkvIWzFDfXiwU5VBbr9gSXZB78jOmAwwEZ"
});
...
app.get("/", async (req, res) => {
    ...
    const user = {
        id: "n.knight.pc0627@gmail.com",
        firstName: "Nknight",
        lastName: "AMAMIYA",
        email: "n.knight.pc0627@gmail.com",
    ...
    const permitted = await permit.check(user.id, "create", "projectv");
    if (permitted) {
        res.status(200).send(`${user.firstName} ${user.lastName} is PERMITTED to create projectv!`);
    } else {
        res.status(403).send(`${user.firstName} ${user.lastName} is NOT PERMITTED to create projectv!`);
    }
});
